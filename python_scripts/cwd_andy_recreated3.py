...
#os.chdir('C:\\Users\\aouimette\\Box\\Terrestrial Ecosystems Lab - Shared All\\pnet_cwd\\Jack_pythonscripts')
...
standing_necromass_bark_ls = []
downed_necromass_bark_ls = []
standing_necromass_bark_N_ls = []
standing_necromass_bark_CN_ls = []
downed_necromass_bark_N_ls = []
downed_necromass_bark_CN_ls = []
...
snag_cohorts_bark = np.zeros((d_yr,d_yr))
dwd_cohorts_bark = np.zeros((d_yr,d_yr))
snag_cohorts_bark_N = np.zeros((d_yr,d_yr))
dwd_cohorts_bark_N = np.zeros((d_yr,d_yr))
snag_cohorts_bark_CN = np.zeros((d_yr,d_yr))
dwd_cohorts_bark_CN = np.zeros((d_yr,d_yr))
...
############### wood biomass ###############  has to come before 
    if i == 0:
        wood_biomass = age.iloc[i]['wood_production'] * (1 - dynam_in['bark_frac_wood'])
    else:
        wood_biomass = (wood_biomass * (1 - age.iloc[i-1]['wood_mort_rate']) +
             age.iloc[i]['wood_production'] * (1 - dynam_in['bark_frac_wood']))

    wood_biomass_ls.append(wood_biomass)   #print('wood_biomass: ', wood_biomass)
...
############### snag necromass ###############
    while j <= i:
        if i == j: # snag_cohorts[i-1, j] == 0: # I think this way it'll never fail
            ...
            snag_cohorts[i,j] = (snag_cohorts[i-1,j] - ((age.iloc[j]['snag_fall_rate'] *
                 snag_cohorts[i-1,j]) + (snag_cohorts[i-1,j] * age.iloc[j]['snag_decay_rate'])))
...
############### snag C/N ###############
        if snag_cohorts_N[i,j] == 0:  #trying to prevent error when there are no snags (divided by zero leads to error)
            snag_cohorts_CN[i,j] = 9999  #trying to prevent error when there are no snags (divided by zero leads to error)
        else:
...
############### dwd Nitrogen ###############
        if i == j: # snag_cohorts[i-1, j] == 0: # I think this way it'll never fail
            ...
            if dwd_cohorts_CN[i-1,j] > dynam_in['critical_CN']:
                ...
                dwd_cohorts_N[i,j] = dwd_cohorts_N[i-1,j] - ((dwd_cohorts[i-1,j] * 0.5 * age.iloc[j]['dwd_decay_rate']) / 
                                           dwd_cohorts_CN[i-1,j]) + (snag_cohorts_N[i-1,j] * age.iloc[j]['snag_fall_rate'])
 
                # formerely (dwd_cohorts_N[i-1,j] * (1 - age.iloc[j]['dwd_decay_rate'])) + (snag_cohorts_N[i-1,j] * age.iloc[j]['snag_fall_rate'])
...
############### The second loop...where i is row (year) and j is col. ###############
for i in range(d_yr):
    #print('istep: ',i)
    #print('Year: ', yr)
    j = 0 #need to reset column each loop
...
############### bark biomass ###############  has to come before 
    if i == 0:
        bark_biomass = age.iloc[i]['wood_production'] * (dynam_in['bark_frac_wood'])
    else:
        bark_biomass = (bark_biomass * (1 - age.iloc[i-1]['wood_mort_rate']) +
             age.iloc[i]['wood_production'] * (dynam_in['bark_frac_wood']))

    bark_biomass_ls.append(bark_biomass)   #print('wood_biomass: ', wood_biomass)


############### snag necromass - bark ###############
    while j <= i:
        if i == j: # snag_cohorts[i-1, j] == 0: # I think this way it'll never fail
            snag_cohorts_bark[i,j] = ((age.iloc[i]['wood_mort_rate'] * bark_biomass_ls[i]) * 
                (1 - age.iloc[j]['snag_fall_rate'])) 
        else: 
            snag_cohorts_bark[i,j] = snag_cohorts_bark[i-1,j] - ((age.iloc[j]['snag_fall_rate'] *
             snag_cohorts_bark[i-1,j]) + (snag_cohorts_bark[i-1,j] * age.iloc[j]['snag_decay_rate']))
    
############### downed necromass - bark ###############
        if i == j:
            dwd_cohorts_bark[i,j] = ((age.iloc[i]['wood_mort_rate'] * bark_biomass_ls[i]) - snag_cohorts_bark[i,j])
        else:
            dwd_cohorts_bark[i,j] = (dwd_cohorts_bark[i-1,j] + (snag_cohorts_bark[i,j] * 
                age.iloc[j]['snag_fall_rate']) - (dwd_cohorts_bark[i-1,j] * age.iloc[j]['dwd_decay_rate']))

############### snag Nitrogen - bark ###############
        if i == j: # snag_cohorts[i-1, j] == 0: # I think this way it'll never fail
            snag_cohorts_bark_N[i,j] = snag_cohorts_bark[i,j] * dynam_in['bark_perc_N'] 
        else:
            if snag_cohorts_bark_CN[i-1,j] > dynam_in['critical_CN']:
                snag_cohorts_bark_N[i,j] = (snag_cohorts_bark_N[i-1,j] * (1 - age.iloc[j]['snag_fall_rate'])) 
            else: #snag_cohorts_CN[i,j] <= dynam_in['critical_CN']:
                snag_cohorts_bark_N[i,j] = (snag_cohorts_bark_N[i-1,j] - ((snag_cohorts_bark[i-1,j]
                    - snag_cohorts_bark[i,j]) * (snag_cohorts_bark_N[i-1,j]/snag_cohorts_bark[i-1,j])))
                             
############### snag C/N - bark ###############
        if snag_cohorts_bark_N[i,j] == 0:  #trying to prevent error when there are no snags (divided by zero leads to error)
            snag_cohorts_bark_CN[i,j] = 9999  #trying to prevent error when there are no snags (divided by zero leads to error)
        else:
            snag_cohorts_bark_CN[i,j] = (snag_cohorts_bark[i,j] * 0.5) / snag_cohorts_bark_N[i,j]

############### dwd Nitrogen - bark ###############
        if i == j: # snag_cohorts[i-1, j] == 0: # I think this way it'll never fail
            dwd_cohorts_bark_N[i,j] = dwd_cohorts_bark[i,j] * dynam_in['bark_perc_N'] 
        else:
            if dwd_cohorts_bark_CN[i-1,j] > dynam_in['critical_CN']:
                dwd_cohorts_bark_N[i,j] = dwd_cohorts_bark_N[i-1,j] + snag_cohorts_bark_N[i-1,j] * age.iloc[j]['snag_fall_rate'] 
            else: #snag_cohorts_CN[i,j] <= dynam_in['critical_CN']:
                dwd_cohorts_bark_N[i,j] = dwd_cohorts_bark_N[i-1,j] - ((dwd_cohorts_bark[i-1,j] * 0.5 * age.iloc[j]['dwd_decay_rate']) / 
                                           dwd_cohorts_bark_CN[i-1,j]) + (snag_cohorts_bark_N[i-1,j] * age.iloc[j]['snag_fall_rate'])
                         
############### dwd C/N - bark ###############
        dwd_cohorts_bark_CN[i,j] = (dwd_cohorts_bark[i,j] * 0.5) / dwd_cohorts_bark_N[i,j]

############### while loop(s) column iterator ###############      
        j += 1   

############### necromass list append (condensed) ###############   
    standing_necromass_bark_ls.append(snag_cohorts_bark[i].sum()) 
    downed_necromass_bark_ls.append(dwd_cohorts_bark[i].sum())
    standing_necromass_bark_N_ls.append(snag_cohorts_bark_N[i].sum())
    standing_necromass_bark_CN_ls.append(snag_cohorts_bark_CN[i].mean())
    downed_necromass_bark_N_ls.append(dwd_cohorts_bark_N[i].sum())
    downed_necromass_bark_CN_ls.append(dwd_cohorts_bark_CN[i].mean())



############### lists to dataframe ###############
age['wood_biomass'] = wood_biomass_ls
age['bark_biomass'] = bark_biomass_ls
...
age['standing_necromass_bark'] = standing_necromass_bark_ls
age['downed_necromass_bark'] = downed_necromass_bark_ls
age['standing_necromass_bark_N'] = standing_necromass_bark_N_ls
age['standing_necromass_bark_CN'] = standing_necromass_bark_CN_ls
age['downed_necromass_bark_N'] = downed_necromass_bark_N_ls
age['downed_necromass_bark_CN'] = downed_necromass_bark_CN_ls
...
age.to_csv('Age_Output.csv')


print('Time to run up through loop:', time.perf_counter() - st)
